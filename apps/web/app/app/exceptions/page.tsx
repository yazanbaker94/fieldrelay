...
export default function ExceptionsPage() {
  return (
    <ConsoleShell active="exceptions" eyebrow="Operational exception queue" title="Exceptions" action={<a className="secondary-button" href="/app/exceptions/EX-0037">Open priority record</a>}>
      ...
      <section className="data-sheet">
        ...
        <div className="data-table exception-table" role="table" aria-label="Exceptions">
          <div className="table-head" role="row"><span>Severity</span><span>Record</span><span>Exception</span><span>Reported variance</span><span>Owner</span><span>Age</span><span>Status</span></div>
          {exceptions.map((exception) => (
            <a href={`/app/exceptions/${exception.id}`} className="table-row" role="row" key={exception.id}>
